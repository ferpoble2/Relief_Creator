...
class TestExportNetcdfAscendingValues(unittest.TestCase):
    ...
    def test_map_ascending_x_values(self):
        exporter = NetcdfExporter()

        # Copy file to modify to not modify the original file
        shutil.copy('resources/test_resources/netcdf/test_file_x_values_descending.nc',
                    'resources/test_resources/temp/temp_file_x_values_ascending.nc')

        # Read the info of the file and generate new data with the same dimension that the one stored in the file
        x, y, z = read_info('resources/test_resources/temp/temp_file_x_values_ascending.nc')

        # Modify the data of the file and read it to compare
        exporter.modify_heights_existent_netcdf_file(z,
                                                     'resources/test_resources/temp/temp_file_x_values_ascending.nc')
        new_x, new_y, new_z = read_info('resources/test_resources/temp/temp_file_x_values_ascending.nc')
