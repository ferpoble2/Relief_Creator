...
FILES_DIRECTORY = './test/filters/files/'
...
class TestHeightGreaterFilter(unittest.TestCase):
    ...
    def test_normal_application(self):
        ...
        self.engine.refresh_with_model_2d(os.path.join(FILES_DIRECTORY, 'colors.cpt'),
                                          os.path.join(FILES_DIRECTORY, 'netcdf', 'test_file_50_50.nc'))

        # load polygon
        self.engine.load_polygon_from_shapefile(os.path.join(FILES_DIRECTORY, 'polygons', 'filter_1'))
        ...
        self.engine.export_model_as_netcdf(self.engine.get_active_model_id(), os.path.join(FILES_DIRECTORY,
                                                                                           'netcdf',
                                                                                           'temp_filter_1'))

        # read data and compare
        info_written = read_info(os.path.join(FILES_DIRECTORY, 'netcdf', 'temp_filter_1.nc'))
        info_expected = read_info(os.path.join(FILES_DIRECTORY, 'netcdf', 'temp_filter_1.nc'))
