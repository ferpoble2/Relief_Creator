...
"""
Module that defines the filter IsIn, filter in charge of selecting the points that are inside a specified polygon.
"""
...
class IsIn(Filter):
    """
    Class in charge of the logic of the IsIn filter.
    """
    name: str = 'Is in'
    ...
    def get_mask(self, model_vertices: 'np.ndarray', mask: 'np.ndarray') -> 'np.ndarray':
        """
        Set True value in the mask to all the points that are inside the specified polygon.
